# -*- coding: utf-8 -*-
"""
Created on Thu Sep 30 21:14:55 2021
...
def generate_plots(test_scores, saving_dir):
    for c in test_scores.columns:
        ...
        plt.ylim([0.0, 1.05])
        plt.tight_layout()
        plt.savefig(os.path.join(saving_dir, str(c)))
    print(test_scores)
...
def plot_box_plot(df, save_dir):
    ...
    for c in df.columns.levels[0]:
        ...
        plt.savefig(os.path.join(save_dir, c + '_boxplot'))
...
def save_confusion_matrix(cnf_matrix, base_dir, model):
    ...
    file_name = os.path.join(base_dir, 'confusion_' + model + '.png')
    plt.savefig(file_name)

    plt.figure()
    plot_confusion_matrix(cnf_matrix, normalize=True, classes=[0, 1],
                          title='Normalized confusion matrix')
    file_name = os.path.join(base_dir, 'confusion_normalized_' + model)
    plt.savefig(file_name)