# -*- coding: utf-8 -*-
"""
Created on Sun Oct 24 23:48:07 2021
...
def boxplot_custom(group_col, val_col, data, ax):
    vals = data.groupby(group_col)[val_col]
...
def plot_activation(ax, column='coef_combined', layer=3, pad=200):
    ...
    df2.variable = df2['variable'].apply(shorten_names)
...
def plot_high_genes2(ax, layer=1, graph='hist', direction='h'):
    ...
    if graph == 'hist':
        ...
        df2 = df2[df2.value != 0]
        boxplot_custom(val_col="value", group_col="variable", data=df2, ax=ax2)
...
def run():
    ...
    plot_activation(ax2, column='coef', layer=3, pad=120)
    
    filename = os.path.join(saving_dir, 'activation_importance.png')
