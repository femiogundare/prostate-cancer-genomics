# -*- coding: utf-8 -*-
"""
Created on Thu Sep 30 08:35:42 2021
...
class Diagonal(keras.layers.Layer):
    def __init__(self, units, activation=None,
        ...
        self.kernel_regularizer = keras.regularizers.get(W_regularizer)
...
class SparseTF(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        #print(type(self.nonzero_ind))
        #print(self.nonzero_ind.shape[0])
        #print(np.array(self.nonzero_ind).shape[0])
    ...
    def get_config(self):
        config = {
            ...
            'W_regularizer': keras.regularizers.serialize(self.kernel_regularizer),
...
class SpraseLayerTF(keras.layers.Layer):
...
class SpraseLayerWithConnection(keras.layers.Layer):
...
class RandomWithMap(keras.initializers.Initializer):
    ...
    def __call__(self, shape, dtype=None):
        map_sparse = scipy.sparse.csr_matrix(self.map)
        ...
        keras.initializers.glorot_uniform().__call__()
        map_sparse.data = init
        return K.variable(map_sparse.toarray())
    
    
    
class L1L2_with_map(keras.regularizers.Regularizer):
