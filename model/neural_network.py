# -*- coding: utf-8 -*-
"""
Created on Thu Sep 30 11:37:37 2021
...
from model.custom_callbacks import GradientCheckpoint, FixedEarlyStopping
from model.model_utils import get_layers, plot_history, get_coef_importance

import sys
sys.path.append(str(Path(__file__).resolve().parents[1]))

from utilities.logs import DebugFolder
from model.builders.prostate_models import build_pnet_account_for
...
class Model(BaseEstimator):
    ...
    def fit(self, X_train, y_train, X_val=None, y_val=None):
        ...
        if type(ret) == tuple:
            #self.model, self.feature_names, self.data_types = ret
        ...
        layer_names_and_outputs = self.get_layer_outputs(X_train)
        layer_names = list(layer_names_and_outputs.keys())
        if self.build_fn == build_pnet_account_for:
            layer_names.remove('combine')
        final_layer_names = layer_names[-self.n_outputs:]
        #print('***************Y shape', np.array(y_train).shape)
        #print('***************Layer names', layer_names)
        #print('****************Final layer names', final_layer_names)
        ...
        history = self.model.fit(X_train, y_train, validation_data=validation_data, epochs=self.nb_epoch,
                                 ...
                                 class_weight={k:v for (k, v) in zip(final_layer_names, [class_weights]*self.n_outputs)}
                                 )
    ...
    def get_named_coef(self):
        ...
        print('Feature nameeeeeeseeeeeeeeeeeeeeeeeeeeeeeeee')
        print(self.feature_names)
        print('\n\n')
        print(self.feature_names.keys())
        print('\n\n')
        print(self.feature_names.values())
    ...
    def get_named_coef(self):
        ...
        #print(len(self.feature_names['inputs']))
        #print(len(self.feature_names['inputs'].ravel()))
        #print(coef['inputs'])
        #print(coef['inputs'].shape)
        #print(self.feature_names['inputs'])
        for k in common_keys:
            c = coef[k]
            names = self.feature_names[k]
            #print(len(names))
            ...
            #    index=[names, data_types]
            #else:
            #    index=[names]
                
            df = pd.DataFrame(c.ravel(), index=names, columns=['coef'])
            #df = pd.DataFrame(c.ravel(), columns=['coef']).set_index(index)
            coef_dfs[k] = df
        return coef_dfs
