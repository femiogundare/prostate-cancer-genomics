# -*- coding: utf-8 -*-
"""
Created on Thu Sep 30 10:38:56 2021
...
def get_shap_scores(model, X_train, y_train, target=-1, method_name='deepexplainer', detailed=False):
    ...
    for l in get_layers(model):
        if type(l) in [keras.models.Sequential, keras.layers.Dropout, keras.layers.BatchNormalization]:
            continue
...
def get_weights_linear_model(model, X, y):
    ...
    inp = model.input
    for i, l in enumerate(layers):
        if type(l) in [keras.models.Sequential, keras.layers.Dropout]:
            continue
        print(type(l))
        if type(l) == keras.layers.InputLayer:
            layer_out = X
...
def get_gradient(model, x, y, x_train, y_train, multiply_by_input=False):
    n_outcomes = 1
